...
## Contents
- `apiClient.js` is the central HTTP client. It applies the API base URL, cookie credentials, JSON handling, and normalized error behavior.
- `apiError.js` defines frontend API error helpers.
- `tokenStorage.js` clears obsolete authentication values previously stored in local storage.
...
## Maintenance Notes
Add domain-specific API calls to the matching service file. Keep low-level HTTP behavior in `apiClient.js` so cookie credentials and error handling stay consistent.
