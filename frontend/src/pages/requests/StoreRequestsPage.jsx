...
function getSupplierName(request) {
  return request.supplier?.companyName || request.supplier?.name || "";
}
...
function StoreRequestsIcon({ name }) {
  const commonProps = {
    width: "20",
    height: "20",
    viewBox: "0 0 24 24",
      ...
      <div className="supplier-request-card__content">
        ...
        <dl className="supplier-request-meta">
          {supplierName && (
            <div>
              <dt>
                <StoreRequestsIcon name="supplier" />
                Fournisseur
              </dt>
              <dd>{supplierName}</dd>
            </div>
          ...
          {request.requestedQuantity && (
            <div>
              <dt>
                <StoreRequestsIcon name="message" />
                Quantité / besoin
              </dt>
          ...
          {requestDate && (
            <div>
              <dt>
                <StoreRequestsIcon name="calendar" />
                Envoyée le
              </dt>
              <dd>{requestDate}</dd>
            </div>
  ...
  useEffect(() => {
    ...
    async function loadRequests() {
      ...
      try {
        ...
        if (shouldUpdateState) {
          setErrorMessage("Impossible de charger les demandes envoyées.");
        }
      } finally {
        if (shouldUpdateState) {
          setIsLoading(false);
    ...
    <div className="supplier-requests-page">
      <header className="supplier-requests-header">
        <div>
          ...
          <p>
            Suivez les demandes de contact et de devis envoyées aux
            fournisseurs.
          </p>
        </div>
        ...
        <>
            ...
            <>
              ...
              >
                <label className="supplier-requests-search">
                  ...
                  <StoreRequestsIcon name="search" />
                  <input
                    name="search"
                    value={filters.search}
                    onChange={handleFilterChange}
                    placeholder="Rechercher un sujet, fournisseur, produit..."
                  />
                </label>
          ...
          {requests.length === 0 ? (
            <EmptyState
              ...
              message="Choisissez un produit ou un fournisseur dans le catalogue pour créer votre première demande."
              action={
            ...
            <section
              ...
              aria-live="polite"
              aria-label="Liste des demandes envoyées"
            >
